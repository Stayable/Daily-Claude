...
A new session is simply a fresh run of `start-claude.bat` — same Daily-Claude repo, same `CLAUDE.md` and chats, clean context. The new window opens its own picker where the user can resume a chat or pick `new`/`quick`.
